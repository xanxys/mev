...
function createPrimitiveKey(primitiveDef) {
	...
	if (dracoExtension) {
		return 'draco:' + dracoExtension.bufferView
...
class GLTFParser {
	/**
	 * @param {Object} json: main glTF strucuture
	 * @param {Map<string, Extension>} extensions: partially decoded extensions
	 */
	constructor(json, extensions) {
		...
		this.textureLoader = new THREE.TextureLoader(THREE.DefaultLoadingManager);
	...
	parse() {
		...
		return Promise.all([
			...
			addUnknownExtensionsToUserData(this.extensions, result, json);
			return result;
		});
	}
	...
	getDependency(type, index) {
		...
		const dep = this._load(type, index);
		this.cache.set(cacheKey, dep);
		return dep;
	}
	...
	_load(type, index) {
		switch (type) {
			...
			case 'buffer':
				return Promise.resolve(this.loadBuffer(index));
			case 'material':
			...
			case 'animation':
				return this.loadAnimation(index);
			case 'camera':
				return this.loadCamera(index);
			case 'light':
				return this.extensions[EXTENSIONS.KHR_LIGHTS_PUNCTUAL].loadLight(index);
			default:
	...
	getDependencies(type) {
		...
		dependencies =
			Promise.all(defs.map((def, index) => parser.getDependency(type, index)));
	...
	loadBuffer(bufferIndex) {
		...
		if (bufferIndex !== 0) {
			throw new Error('THREE.GLTFLoader: ' + bufferIndex + ' GLB only has bufferIndex=0.');
		}
	...
	loadTexture(textureIndex) {
		...
		if (source.bufferView === undefined) {
			throw new Error("Only bufferView is supported for texture load source", source);
		}

		// Load binary image data from bufferView, if provided.
		return this.getDependency('bufferView', source.bufferView)
			.then(bufferView =>
	...
	loadGeometries(primitives) {
		...
		for (var i = 0, il = primitives.length; i < il; i++) {
			...
			if (!cache.has(cacheKey)) {
				...
				// Cache this geometry
				cache.set(cacheKey, { primitive: primitive, promise: geometryPromise });
			}

			pending.push(cache.get(cacheKey).promise);
		}
		return Promise.all(pending);
	}
	...
	loadMesh(meshIndex) {
		...
		const pending = primitives.map(primitive => {
			return primitive.material === undefined
				? createDefaultMaterial()
				: this.getDependency('material', primitive.material);
		});
	...
	loadNode(nodeIndex) {
		...
		return (() => {
			...
			if (nodeDef.isBone === true) {
				...
				return this.getDependency('camera', nodeDef.camera);
			} else if (nodeDef.extensions
				&& nodeDef.extensions[EXTENSIONS.KHR_LIGHTS_PUNCTUAL]
				&& nodeDef.extensions[EXTENSIONS.KHR_LIGHTS_PUNCTUAL].light !== undefined) {

				return this.getDependency('light', nodeDef.extensions[EXTENSIONS.KHR_LIGHTS_PUNCTUAL].light);
			} else {
	...
	loadScene(sceneIndex) {
		...
		const pending = nodeIds.map(nodeId => buildNodeHierachy(nodeId, scene, json, this));
...
function buildNodeHierachy(nodeId, parentObject, json, parser) {
	...
	return parser.getDependency('node', nodeId).then(node => {
		...
		return parser.getDependency('skin', nodeDef.skin).then(skin => {
			skinEntry = skin;
			return Promise.all(skinEntry.joints.map(joint => parser.getDependency('node', joint)));
		}).then(jointNodes => {
...
function addPrimitiveAttributes(geometry, primitiveDef, parser) {
	...
	if (primitiveDef.indices !== undefined && !geometry.index) {
		var accessor = parser.getDependency('accessor', primitiveDef.indices).then(accessor => {
